...
// Translates a Windows-style module name to a platform shared object and dlopen()s it.
...
void* Sys_dlopen(const char* name)
{
	...
	// Translate a trailing ".dll" to the platform extension, or append it if the basename has no extension.
	const size_t len = strlen(path);
	if (len > 4 && Q_stricmp(path + len - 4, ".dll") == 0)
	{
		strcpy(path + len - 4, ext);
	}
	else
	{
